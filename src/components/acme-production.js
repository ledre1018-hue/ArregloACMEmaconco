...
class AcmeProduction extends HTMLElement {
    ...
    setupListeners() {
        ...
        // Cambio de cantidad: refrescar la vista previa con el nuevo calculo
        // (si no se escucha este evento, el preview se queda con los numeros
        // de la cantidad anterior y puede confundir al usuario)
        const cantInput = this.querySelector('#prod-cantidad');
        if (cantInput && select) {
            cantInput.addEventListener('input', () => this.mostrarPreviewFormula(select.value));
        }
    ...
    async ejecutarProduccion() {
        ...
        for (const ingrediente of producto.formula) {
            ...
            if (idx !== -1) {
                ...
                console.log(`[Production] MP ${mp.codigo}: ${mp.stock || 0} -> ${nuevoStock} (consumio ${consumo})`);
            }
        }
