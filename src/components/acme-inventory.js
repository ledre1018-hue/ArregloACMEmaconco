...
class AcmeInventory extends HTMLElement {
            ...
            <div class="card mb-3">
                ...
                <div class="card-body">
                    <form id="inv-form">
                        ...
                        <div class="form-row">
                            <div class="form-group">
                                <label>Stock ${this.editandoKey ? 'Actual' : 'Inicial'}</label>
                                <input type="number" id="inv-stock" placeholder="0" value="0" min="0" step="0.01"
                                    ${this.editandoKey ? 'disabled' : ''}>
                                ${this.editandoKey ? '<small class="help-text">El stock no se edita aquí: use "Ingresar al Inventario", "Producción" o "Registrar Merma" según el caso.</small>' : ''}
                            </div>
                            <div class="form-group">
    ...
    async guardarProducto() {
        ...
        try {
            ...
            // IMPORTANTE: primero se refrescan los datos (this.materiasPrimas
            // queda actualizado con lo recien creado) y SOLO DESPUES se repinta
            // el formulario. Si esto se hace al reves, la fila de formula que
            // se agrega por defecto se construye con la lista vieja de materias
            // primas y la recien creada no aparece como opcion para elegir.
            await this.cargarDatos();
            this.render();
            // render() reconstruye el esqueleto de la tabla con el texto
            // "Cargando..." de nuevo; como los datos frescos ya estan en
            // memoria (this.productosFiltrados), se repinta de inmediato
            // sin necesidad de otra llamada a Firebase. Los selects de
            // ingreso/merma y la seccion de formula ya se refrescan solos
            // dentro de render(), asi que no hace falta repetirlos aqui.
            this.renderTabla(this.productosFiltrados);
